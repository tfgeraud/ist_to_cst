...
ISTSharingTree *ist_compute_predecessor_paths_tree(S, period)
	ISTSharingTree *S;
	int *period;
{
	struct LOC_ist_method  V;
	...
	s1 = S->Root->FirstSon;
	while (s1 != NULL) {       
		rchild = predecessorPathsWithinTree(s1->Son, period, &V, 0);
		if (rchild != NULL)
			ist_add_son(V.STR->Root, rchild);
		s1 = s1->Next;
	}
...
ISTNode *predecessorPathsWithinTree(node1, period, LINK, pos)
        ISTNode *node1;  
        int *period, pos; 
        struct LOC_ist_method *LINK;
{
...
// 			   nnode->Info->Right = INFINITY;
		...
		//	   ist_dispose_node(mnode);
			   ...
			   while (s2 != NULL){
			     pos=pos + 1;
			     rchild = predecessorPathsWithinTree(s2->Son, period, LINK, pos);
			     printf("i am here 33 \n");
			     if(rchild != NULL){
