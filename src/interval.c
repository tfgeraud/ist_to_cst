...
// inline boolean ist_is_condition_nat(val1, val2 )
// 	ISTInterval *val1, *val2;
// {
//     boolean result = false;
...
inline boolean ist_is_condition_nat_rct(val1, val2, val3, is_Nat)
	ISTInterval *val1, *val2, *val3;
	boolean is_Nat ;
{
    boolean result = false;	
    if (is_Nat){
        if (val2->Left == val3->Left)
           result = true;
    } else{
       if (((val1->Left == 0) && (val2->Left >= val3->Left)) || ((val1->Left != 0) && (val2->Left == val3->Left)))
           result = true;
    }
    return result;
}
