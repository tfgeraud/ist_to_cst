...
/*
static ISTNode *SubsumedPathsWithinTreeNAT(node1, node2, DifferentNode, LINK)
	ISTNode *node1, *node2;
	boolean DifferentNode;
	struct LOC_ist_method  *LINK;
{
	...
	if (ist_equal_interval(node1->Info,&IST_end_of_list)) {
		...
		while (s1 != NULL) {
			s2 = node2->FirstSon;
			while (s2 != NULL) {
				if (ist_is_condition_nat(s2->Son->Info,s1->Son->Info)) {
					/* s1 \subset s2 */
					/*if (s1->Son != s2->Son)
						NewDifferentNode = true;
					else
					...
					if (LINK->memo != NULL)
						...
						rchild = SubsumedPathsWithinTreeRCT(s1->Son, s2->Son, NewDifferentNode, LINK);
					if (rchild != NULL)
						ist_add_son(rnode, rchild);
					s2 = s2->Next;
				} else {
...
static ISTNode *SubsumedPathsWithinTreeNAT_RCT(node1, node2, DifferentNode, LINK, is_nat)
	ISTNode *node1, *node2;
	boolean DifferentNode, is_nat;
	struct LOC_ist_method  *LINK;
{
	ISTSon *s1, *s2;
	ISTNode *rnode, *rchild;
	boolean NewDifferentNode, is_cond, memo_nat;
	ISTInterval *interv = NULL;
	memo_nat = is_nat;
	...
	if (ist_equal_interval(node1->Info,&IST_end_of_list)) {
		...
		while (s1 != NULL) {
			...
			while (s2 != NULL) {
				if (is_nat){
				  is_cond= ist_is_condition_nat_rct (NULL, s1->Son->Info,s2->Son->Info, true);
// 				  is_nat = false;
				} else{
				  is_cond= ist_is_condition_nat_rct (node1->Info, s1->Son->Info,s2->Son->Info, false);
// 				  is_nat = true;
				}
				if (is_cond){
				    is_nat = ((is_nat)? false : true);
					/* s1 \subset s2 */
					if (s1->Son != s2->Son)
						NewDifferentNode = true;
					else
					...
					if (LINK->memo != NULL)
						...
						rchild = SubsumedPathsWithinTreeNAT_RCT(s1->Son, s2->Son, NewDifferentNode, LINK, is_nat);
						//is_nat = ((is_nat)? false : true);}
						is_nat = memo_nat;
	...
	ISTSharingTree *S;
{
	...
	while (s1 != NULL) {
		...
		while (s2 != NULL) {
				...
				/* s1 \subset s2 */
				if (s1->Son != s2->Son)
					rchild = SubsumedPathsWithinTreeNAT_RCT(s1->Son, s2->Son, true, &V, false);
				else
					rchild = SubsumedPathsWithinTreeNAT_RCT(s1->Son, s2->Son, false, &V, false);
				if (rchild != NULL)
					ist_add_son(V.STR->Root, rchild);
				s2 = s2->Next;
		}
...
ISTSharingTree* ist_compute_predecessor_paths_tree_request(S, period)
	ISTSharingTree *S;
	int *period;
{
	struct LOC_ist_method  V;
	...
	s1 = S->Root->FirstSon;
	while (s1 != NULL) {       
		rchild = predecessorPathsWithinTreeRequest(s1->Son, period, &V, 0);
		if (rchild != NULL)
			ist_add_son(V.STR->Root, rchild);
		s1 = s1->Next;
	}
...
ISTNode *predecessorPathsWithinTreeRequest(node1, period, LINK, pos)
        ISTNode *node1;  
        int *period, pos; 
        struct LOC_ist_method *LINK;
{
   	ISTSon *s1, *s2;
	ISTNode *rnode, *rchild, *nnode, *mnode, *sol ;
	boolean NewDifferentNode, flag=false; 
	ISTInterval * interv;
	
	if (ist_equal_interval(node1->Info,&IST_end_of_list)) {
		printf("i have finished \n");
		rnode = ist_add_node(LINK->rlayer, ist_create_node(&IST_end_of_list));
		printf("i have finished2 \n");
		//sol = rnode;
		return rnode;
	} else {
		rnode = ist_create_node(node1->Info);
		...
		while (s1 != NULL) 
                {
			if (s1->Son->Info->Left == period[pos])
			{
			 LINK->memo = ist_get_memoization1(s1->Son,NULL);
			 if (LINK->memo != NULL)
				rchild = LINK->memo->r;
			 else{
			   interv = ist_build_interval(0, INFINITY);
			   nnode= ist_create_node(interv);
// 			   nnode->Info->Left = 0;
// 			   nnode->Info->Right = INFINITY;
			   //mnode= ist_new_node();
			   //mnode->Info->Left = 0;
			   //mnode->Info->Right = INFINITY;
			   mnode = ist_create_node(ist_build_interval(0, INFINITY));
			   printf("i am here 22 \n");
			   ist_add_son(nnode, mnode);
			   printf("i am here 2 \n");
		//	   ist_dispose_node(mnode);
			   s2 = s1->Son->FirstSon;
			   while (s2 != NULL){
			     pos=pos + 1;
			     rchild = predecessorPathsWithinTreeRequest(s2->Son, period, LINK, pos);
			     printf("i am here 33 \n");
			     if(rchild != NULL){
			     ist_add_son(mnode, rchild);
			     printf("i am here 3 \n");}
			     s2 = s2->Next;
			     }
                }
			  ...
			  if (LINK->memo != NULL)
			   ...
			   mnode = ist_create_node( s1->Son->Info);
// 			   mnode = ist_new_node();
// 			   mnode->Info->Left = s1->Son->Info->Left;
// 			   mnode->Info->Right=s1->Son->Info->Right;
			   printf("i am here 44 \n");
			   ist_add_son(rnode, mnode);
			   printf("i am here 4 \n");
		//	   ist_dispose_node(mnode);
			   s2 = s1->Son->FirstSon;
			   while (s2 != NULL){
...
// 	   ist_dispose_node(rnode);
	   sol=nnode;
// 	   ist_dispose_node(nnode);
	}
	ist_put_memoization1(node1,NULL, sol);
	return sol;
}

ISTNode *predecessorPathsWithinTreeRun(node1, period, LINK, pos)
	ISTNode *node1;
	int *period, pos; 
        struct LOC_ist_method *LINK;
{	
	...
	if (ist_equal_interval(node1->Info,&IST_end_of_list)) {
		...
		while (s1 != NULL) 
			...
			if (s1->Son->Info->Left == period[pos])
			{
			 ...
			 if (LINK->memo != NULL)
			   ...
			   nnode= ist_create_node(ist_build_interval(0, INFINITY));
			   mnode = ist_create_node(ist_build_interval(0, INFINITY));
			   printf("i am here 22 \n");
			   ist_add_son(nnode, mnode);
			   printf("i am here 2 \n");
		//	   ist_dispose_node(mnode);
			   ...
			   while (s2 != NULL){
			     pos=pos + 1;
			     rchild = predecessorPathsWithinTreeRun(s2->Son, period, LINK, pos);
			     printf("i am here 33 \n");
			     if(rchild != NULL){
			     ist_add_son(mnode, rchild);
			     printf("i am here 3 \n");}
...
ISTSharingTree *ist_compute_predecessor_paths_tree_run(S, period)
	ISTSharingTree *S;
	int *period;
{
	struct LOC_ist_method  V;
	...
	s1 = S->Root->FirstSon;
	while (s1 != NULL) {       
		rchild = predecessorPathsWithinTreeRun(s1->Son, period, &V, 0);
		if (rchild != NULL)
			ist_add_son(V.STR->Root, rchild);
		s1 = s1->Next;
	}
